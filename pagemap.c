...
// Mapa wszystkich stron pamięci 4GB
uint8_t page_map[PAGE_COUNT/4];

// Inicjuje mape stron
void pagemap_initialize(void)
{
    printf("Page Map Initialization\n");
    ...
    struct multiboot_mmap_entry *entry = (struct multiboot_mmap_entry *)multiboot_get_info()->mmap_addr;
    uint32_t count = multiboot_get_info()->mmap_length / sizeof(struct multiboot_mmap_entry);
...
{
    for(uint32_t i=0; i<PAGE_COUNT-count+1; i++)
    {
        int ok_flag = 1;
...
static void page_set_from_index(uint32_t nr, uint32_t state)
{
    ...
    uint8_t *page_byte = page_map + byte;
